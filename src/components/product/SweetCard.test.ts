...
it.each([0,15,25])('shows Bobbatlu preparation according to stock %s and selected box size',async count=>{
  vi.stubGlobal('IS_REACT_ACT_ENVIRONMENT',true);
  vi.stubGlobal('fetch',vi.fn(async()=>Response.json({stock:{'sweet-bobbatlu':count}})));
  invalidateStock();
